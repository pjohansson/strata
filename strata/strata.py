...
cmd_intsample = {
        'name': 'sample',
        'desc': 'Sample length and integrated area of interfaces.'
}
...
@interface.command(name=cmd_intsample['name'], short_help=cmd_intsample['desc'])
@add_argument('base', type=str)
@add_argument('variable',
    type=click.Choice(['area', 'length']))
@add_option('-o', '--save_fig', type=str, default=None,
        help='Save figures to base path..')
@add_option('-x', '--save_xvg', type=click.Path(), default='',
        help='Save collected data to disk at base as .xvg file.')
...
@add_option('--extfig', default='.png',
        help='Save figures using this file extension. (.png)')
def interface_sample_cli(base, variable, **kwargs):
    """Sample input VARIABLE from interfaces of input BASE.
    ...
    if not kwargs['ylabel']:
        if variable == 'area':
            kwargs['ylabel'] = "A (nm^2)"
        else:
            kwargs['ylabel'] = "L (nm)"

    set_none_to_inf(kwargs)
    sample_interfaces(base, variable, **kwargs)
